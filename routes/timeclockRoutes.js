...
// Inside backend routes/timeclockRoutes.js:
...
// ==========================================
router.put('/admin/:id/approve', authenticateToken, requirePermission('employees:edit'), asyncHandler(async (req, res) => {
...
router.post('/admin/create', authenticateToken, requirePermission('employees:edit'), asyncHandler(async (req, res) => {
  const { employeeId, date, checkInTime, checkOutTime, breakMinutes, shiftType } = req.body;
  ...
  if (shiftType === 'conge') {
    ...
    if (!checkInTime || !checkOutTime) {
      return res.status(400).json({ message: 'Les heures d arrivée et de départ sont requises pour un shift de travail.' });
    }
...
// ==========================================
router.put('/admin/:id', authenticateToken, requirePermission('employees:edit'), asyncHandler(async (req, res) => {
...
// ==========================================
router.put('/admin/:id/unlock', authenticateToken, requirePermission('employees:edit'), asyncHandler(async (req, res) => {
  const timesheet = await Timeclock.findById(req.params.id);
...
// ==========================================
router.delete('/admin/:id', authenticateToken, requirePermission('employees:delete'), asyncHandler(async (req, res) => {
  const timesheet = await Timeclock.findById(req.params.id);
  if (!timesheet) return res.status(404).json({ message: 'Fiche introuvable.' });
