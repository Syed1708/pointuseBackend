...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (currentUser.role.name !== "admin") {
      ...
      if (hasUnauthorizedPermission) {
        return res.status(403).json({
          message:
            "You cannot assign privileges to a role that exceed your own permissions.",
        });
      }
    }
    ...
    // 🔌 Emit Real-Time Broadcast [2]
    req.app.get("io").emit("role_updated");
  ...
  asyncHandler(async (req, res) => {
    ...
    if (role.name === "admin") {
      return res
        .status(403)
        .json({
          message: "The primary Admin role cannot be modified or renamed.",
        });
    }
    ...
    if (currentUser.role.name !== "admin") {
      ...
      if (hasUnauthorizedPermission) {
        return res.status(403).json({
          message:
            "You cannot grant privileges to a role that exceed your own permissions.",
        });
      }
    }
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (role.name === "admin") {
      return res
        .status(403)
        .json({ message: "The primary Admin role cannot be deleted." });
    }
